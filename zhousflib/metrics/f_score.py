...
class EvalScore(object):
    ...
    def __init__(self, y_true: list, y_pre: list, f_beta: float = 1):
        self.y_true = y_true
        self.y_pre = y_pre
        self.f_beta = f_beta
    ...
    def print(self):
        print("tn={0}\nfp={1}\nfn={2}\ntp={3}".format(self.tn, self.fp, self.fn, self.tp))
        print("recall={0}\nprecision={1}".format(self.recall, self.precision))
        print("f_{0}_score={1}".format(self.f_beta, self.f_beta_score))
    ...
    def f_beta_score_compute(self, f_beta: float, y_true: list = None, y_pre: list = None):
        """
        ...
        if y_pre is None:
            y_pre = self.y_pre
        f_score = fbeta_score(y_true=y_true, y_pred=y_pre, beta=f_beta)
...
if __name__ == "__main__":
    ...
    score = EvalScore(y_true=actual_labels, y_pre=predicted_labels, f_beta=2)
    score.print()
